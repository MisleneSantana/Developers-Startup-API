...
describe('PATCH - /projects/:id', () => {
  ...
  it('Sucesso - Atualizando um projeto com sucesso.', async () => {
    ...
    const projectResponse = await supertest(app).post('/projects').send(project);

    const response = await supertest(app).patch(`/projects/${projectResponse.body.id}`).send(projectUpdate1);
  ...
  it('Falha - Tentando atualizar um projeto com um id de projeto inválido.', async () => {
    const response = await supertest(app).patch(`/projects/${999}`).send(projectUpdate2);
